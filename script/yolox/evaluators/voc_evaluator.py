...
class VOCEvaluator:
    ...
    def convert_to_voc_format(self, outputs, info_imgs, ids):
        ...
        for output, img_h, img_w, img_id in zip(outputs, info_imgs[0], info_imgs[1], ids):
            ...
            if outputs[0].shape[1]==11:
                angles=output[:, 4:8]
                predictions[int(img_id)] = (bboxes, cls, scores ,angles)
            else:
    ...
    def evaluate_prediction(self, data_dict, statistics):
        ...
        for img_num in range(self.num_images):
            if len(data_dict[0])==4:
                bboxes, cls, scores ,angles= data_dict[img_num]
            else :
            ...
            if bboxes is None:
                for j in range(self.num_classes):
                    all_boxes[j][img_num] = np.empty([0, num_out], dtype=np.float32)
            ...
            for j in range(self.num_classes):
                ...
                c_dets = torch.cat((bboxes,angles ,scores.unsqueeze(1)), dim=1)
