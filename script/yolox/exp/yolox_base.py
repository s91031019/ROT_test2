...
class Exp(BaseExp):
    ...
    def get_model(self):
        from yolox.models import YOLOX, YOLOPAFPN, YOLOXHead ,RotateHead
        ...
        if getattr(self, "model", None) is None:
            ...
            if self.rotate:
                head = RotateHead(self.num_classes, self.width, in_channels=in_channels, act=self.act)
            else:
                head = YOLOXHead(self.num_classes, self.width, in_channels=in_channels, act=self.act)
            self.model = YOLOX(backbone, head ,ROT=self.rotate)
