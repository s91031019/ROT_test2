...
def _mirror_R(image, boxes, prob=0.5):
    #flap 180
...
class TrainTransform:
    def __init__(self, max_labels=80, flip_prob=0.5, hsv_prob=1.0):
    ...
    def __call__(self, image, targets, input_dim):
        ...
        if random.random() < self.hsv_prob:
            augment_hsv(image)
        if targets.shape[1]==9:
            image_t, boxes = _mirror_R(image, boxes, self.flip_prob)
        else:
