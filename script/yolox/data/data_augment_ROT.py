...
import cv2
import numpy as np
from yolox.data.datasets.mosaicdetection import test_img
...
def test_img(img,labels=None):
    ...
    CLASSES=["T1","T2","T3","T4","T5","T6","T7","T8","T9","T10","T11","T12","T13"]
    if labels is not None:
        for i in labels:
            ...
            cv2.putText(img,CLASSES[int(i[8])],(int(i[0]),int(i[1])),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,255),1,cv2.LINE_AA)
    cv2.imshow("aa",img)
    cv2.waitKey(0)
...
class TrainTransform_R:
    ...
    def __call__(self, image, targets, input_dim):
        ...
        image_t, r_ ,r_img= preproc(image_t, input_dim)
