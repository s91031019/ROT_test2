...
class RotateHead(nn.Module):
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv,stride_this_level, x) in enumerate(
            ...
            ang_output=self.angle_preds[k](reg_feat)
    ...
    def get_losses(
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                ang_target = outputs.new_zeros((0, 4))
                l1_target = outputs.new_zeros((0, 4))
                obj_target = outputs.new_zeros((total_num_anchors, 1))
                fg_mask = outputs.new_zeros(total_num_anchors).bool()
            else:
                gt_bboxes_per_image = labels[batch_idx, :num_gt, 1:5]
                gt_angles=labels[batch_idx,:num_gt,5:9]
        ...
        # a=ang_preds[:,:,2:4].view(-1,2)[fg_masks]
        loss_direct = (
            
            self.bcewithlog_loss(ang_preds[:,:,2:4].view(-1,2)[fg_masks], ang_targets[:,2:4])
        ).sum() / num_fg

        loss_R=(
            torch.pow(ang_preds[:,:,0:2].view(-1, 2)[fg_masks]-ang_targets[:,0:2],2)
        ).sum() / num_fg
        ...
        reg_weight = 5.0
        loss_ang=0.750*loss_direct+1.5*loss_R
        # loss_ang=0.0
    ...
    def get_assignments(
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            angl_preds_=(
                angl_preds_.float().sigmoid_())
            # a=angl_preds_[:,2:4].unsqueeze(0).repeat(num_gt, 1, 1)
            # b= gt_angles[:,2:4].unsqueeze(1).repeat(1, num_in_boxes_anchor, 1)
            direct_loss = F.binary_cross_entropy(
                angl_preds_[:,2:4].unsqueeze(0).repeat(num_gt, 1, 1),
                gt_angles[:,2:4].unsqueeze(1).repeat(1, num_in_boxes_anchor, 1),
                 reduction="none"
            ).sum(-1)
            Rx_loss = F.mse_loss(
                angl_preds_[:,:2].unsqueeze(0).repeat(num_gt, 1, 1),
                gt_angles[:,:2].unsqueeze(1).repeat(1, num_in_boxes_anchor, 1),
                 reduction="none"
            ).sum(-1)

            pair_wise_ang_loss=0.15*Rx_loss+0.02*direct_loss
        ...
        del pair_wise_cls_loss, cost, pair_wise_ious, pair_wise_ious_loss,pair_wise_ang_loss
